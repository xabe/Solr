...
import static com.xabe.spring.solr.domain.entity.VehicleTypeDO.CAR;
...
class CarConverterTest {
  ...
  private final CarDO carDO =
      CarDO.builder().vehicleVersion(1).updateVersion(1L).visibilityVersionTimestamp(2L)
          .id(VehicleIdDO.builder().type(CAR).id("id").build()).brand("brand")
          ...
          .doors(4)
          .type(CarTypeDO.COUPE)
          .updateInstants(Map.of("basic", this.in)).build();

  private VehicleConverter<CarDO> vehicleConverter;

  @BeforeEach
  public void setUp() throws Exception {
    this.gson = new Gson();
    this.vehicleConverter = new CarConverter();
  }

  @Test
  public void getType() throws Exception {
    assertThat(this.vehicleConverter.getType(), is(CAR));
  }

  @Test
  public void givenASolrDocumentWhenInvokeReaderFromSolrThenReturnCarDO() throws Exception {
    //Given
    final SolrDocument solrDocument = new SolrDocument();
    solrDocument.setField(VEHICLE_ID, "id");
    solrDocument.setField(VEHICLE_TYPE, "CAR");
    ...
    solrDocument.setField(ENGINES_BLOB, this.gson.toJson(this.carDO.getEngines()));
    solrDocument.setField(PRICES_BLOB, this.gson.toJson(this.carDO.getPrices().get()));
    solrDocument.setField(WHEELS_BLOB, this.gson.toJson(this.carDO.getWheels()));
    solrDocument.setField(TRANSMISSIONS_TYPE, List.of("AUTOMATIC", "MANUAL"));
    solrDocument.setField(COLORS_BLOB, this.gson.toJson(this.carDO.getColors()));
    solrDocument.setField(DOORS, 4);
    solrDocument.setField(CAR_BODY_TYPE, "COUPE");

    //When
    final CarDO result = this.vehicleConverter.readerFromSolr(solrDocument);

    //Then
    assertThat(result, is(notNullValue()));
    assertThat(result, is(this.carDO));
  }

  @Test
  public void givenACarDOWhenInvokeWriteToSolrThenReturnSolrDocumentInput() throws Exception {
    //Given

    //When
    final SolrInputDocument result = this.vehicleConverter.writeToSolr(this.carDO);

    //Then
    assertThat(result, is(notNullValue()));
    assertThat(result.getFieldValue(ID), is("sp!CAR::id::1"));
    assertThat(result.getFieldValue(GROUP_ID_FIELD), is("CAR::id"));
    assertThat(result.getFieldValue(STORE_ID), is("sp"));
    assertThat(result.getFieldValue(VEHICLE_ID), is("id"));
    assertThat(result.getFieldValue(VEHICLE_TYPE), is("CAR"));
    ...
    assertThat(result.getFieldValue(ENGINES_BLOB), is(this.gson.toJson(this.carDO.getEngines())));
    assertThat(result.getField(PRICES).getValue(), is(Set.of(100L)));
    assertThat(result.getFieldValue(VEHICLE_HAS_PRICES), is(true));
    assertThat(result.getFieldValue(PRICES_BLOB), is(this.gson.toJson(this.carDO.getPrices().get())));
    ...
    assertThat(result.getFieldValue(WHEELS_BLOB), is(this.gson.toJson(this.carDO.getWheels())));
    assertThat(result.getField(TRANSMISSIONS_TYPE).getValue(), is(Set.of("MANUAL", "AUTOMATIC")));
    assertThat(result.getFieldValue(COLORS_BLOB), is(this.gson.toJson(this.carDO.getColors())));
    ...
    assertThat(result.getFieldValue(DOORS), is(4));
    assertThat(result.getFieldValue(CAR_BODY_TYPE), is("COUPE"));
  }
